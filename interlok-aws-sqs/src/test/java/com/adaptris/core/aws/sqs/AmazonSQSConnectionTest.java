...
public class AmazonSQSConnectionTest extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    amazonSQSConnection.setAccessKey("accessKey");
    amazonSQSConnection.setSecretKey("secretKey");
  ...
  public void testInitWithoutCredentials() throws Exception {
    when(mockSqsClientFactory.createClient((AWSCredentials) anyObject())).thenReturn(mockSqsClient);
    
    amazonSQSConnection.setAccessKey(null);
    amazonSQSConnection.setSecretKey(null);
    
    try {
      amazonSQSConnection.init();
      fail("Should fail without crdentials");
    } catch (CoreException ex) {
      //expected
    }
  }
